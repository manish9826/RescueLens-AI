...
const SAMPLE_INCIDENTS = [
  {
    ...
    icon: Droplets,
    color: 'border-blue-200 dark:border-blue-500/40 text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40',
  ...
  },
  {
    ...
    icon: Flame,
    color: 'border-red-200 dark:border-red-500/40 text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-950/40',
  ...
  },
  {
    ...
    icon: Car,
    color: 'border-amber-200 dark:border-amber-500/40 text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40',
  ...
  },
  {
    ...
    icon: Building2,
    color: 'border-purple-200 dark:border-purple-500/40 text-purple-700 dark:text-purple-400 bg-purple-50 dark:bg-purple-950/40',
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 py-4">
      ...
      <div className="text-center space-y-2">
        <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-red-50 dark:bg-red-950/80 border border-red-200 dark:border-red-500/40 text-red-700 dark:text-red-300 text-xs font-mono font-bold">
      ...
      {/* Main Upload Card */}
      <div className="glass-card rounded-3xl p-6 sm:p-10 space-y-8 border border-slate-200 dark:border-slate-800 relative shadow-xl">
        
        {/* Multiphase Loading Overlay */}
        {isLoading && (
          <div className="absolute inset-0 z-30 bg-slate-950/95 backdrop-blur-xl rounded-3xl flex flex-col items-center justify-center p-6 text-center space-y-6">
        ...
        {/* Error Alert */}
        {error && (
          <div className="p-4 bg-red-50 dark:bg-red-950/80 border border-red-200 dark:border-red-500/40 rounded-2xl text-red-900 dark:text-red-200 text-xs flex items-center gap-3 shadow-sm">
            <AlertCircle className="w-5 h-5 text-red-600 dark:text-red-400 shrink-0" />
            <div>
              <strong className="block font-bold text-red-950 dark:text-red-300">Analysis Error</strong>
              {error}
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-3">
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider font-mono">
              1. Emergency Scene Photo <span className="text-red-600 dark:text-red-400">*</span>
            </label>
              ...
              <div
                ...
                className={`border-2 border-dashed rounded-3xl p-8 sm:p-14 text-center cursor-pointer transition-all duration-300 ${
                  isDragOver
                    ...
                    : 'border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/40 hover:border-cyan-500 hover:bg-slate-100/50 dark:hover:bg-slate-900/80'
                }`}
              >
                <input
                  type="file"
                ...
                <div className="flex flex-col items-center gap-4">
                  <div className="w-16 h-16 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-cyan-600 dark:text-cyan-400 flex items-center justify-center shadow-md">
                    <Upload className="w-8 h-8" />
                  </div>
                  <div>
                    <span className="text-slate-900 dark:text-slate-100 font-bold text-lg block">
                      Drag & drop emergency scene photo here
                    </span>
                    <span className="text-slate-500 dark:text-slate-400 text-xs mt-1 block font-mono">
                      Supports JPG, PNG, WEBP (or click to browse from device)
                    </span>
                  </div>
                  <button
                    type="button"
                    className="mt-2 px-5 py-2.5 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2 shadow-sm"
                  >
                    <Camera className="w-4 h-4 text-cyan-600 dark:text-cyan-400" /> Browse Photo Files
                  </button>
                </div>
              </div>
            ) : (
              <div className="relative rounded-3xl overflow-hidden border border-slate-300 dark:border-slate-700 bg-slate-950 group shadow-xl">
                <img
                  src={selectedImage}
              ...
              {SAMPLE_INCIDENTS.map((sample) => {
                ...
                return (
                  <button
                    ...
                    className={`p-3.5 rounded-2xl border text-left transition-all hover:scale-[1.02] flex flex-col justify-between space-y-2 shadow-sm ${sample.color}`}
                  >
          ...
          <div className="space-y-2">
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider font-mono flex items-center gap-2">
            ...
            <textarea
              ...
              className="w-full bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-2xl p-4 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-colors shadow-inner"
            />
          </div>
          ...
          <button
            ...
            className={`w-full py-4 rounded-2xl font-extrabold text-base flex items-center justify-center gap-3 transition-all shadow-lg ${
              selectedImage && !isLoading
                ? 'bg-red-600 hover:bg-red-500 text-white shadow-red-600/30 transform hover:-translate-y-0.5'
